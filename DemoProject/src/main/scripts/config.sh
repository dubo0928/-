...
########################################################## set java opts ###########################################################
JAVA_OPTS="-Dapp.home=$APP_HOME -Dstdout.print.2console=false -Dspring.config.location=classpath:/,classpath:/config/,file:$ROOT_DIR/config/,file:$ROOT_DIR/config/,file:$ROOT_DIR/config/application-prod.properties -Dspring.profiles.active=prod -XX:-OmitStackTraceInFastThrow $JAVA_OPTS"
